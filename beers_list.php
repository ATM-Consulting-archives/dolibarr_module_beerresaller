...
/**
 *   	\file       beerresaler/beers_list.php
 *		\ingroup    beerresaler
...
$help_url='';
$title = 'Bières en stock :' ; // $langs->trans('MyModuleListTitle');
...
$sql.= " FROM ".MAIN_DB_PREFIX."beers as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."beers_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_enStock) $sql.= natural_search("enStock",$search_enStock);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/beers_card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.name']['checked'])) print_liste_field_titre($arrayfields['t.name']['label'],$_SERVER['PHP_SELF'],'t.name','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.enStock']['checked'])) print_liste_field_titre($arrayfields['t.enStock']['label'],$_SERVER['PHP_SELF'],'t.enStock','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.enStock']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_enStock" value="'.$search_enStock.'" size="10"></td>';
...
while ($i < min($num, $limit))
{
    ...
    if ($obj)
    {
        ...
        {
            print '<td><a href="./beers_card.php?action=edit&id='.$obj->rowid.'">'.$obj->name.'</a></td>';
        ...
        {
            print '<td>'.$obj->prix.'</td>';
		    if (! $i) $totalarray['nbfield']++;
        }
			 if (! empty($arrayfields['t.enStock']['checked'])) 
        {
            print '<td>'.$obj->enStock.'</td>';
		    if (! $i) $totalarray['nbfield']++;
        }
